...
import { impactApi, type SuccessStory, type ImpactMetric } from '../services/impactApi';
...
const Donate = () => {
  ...
  // Fetch donation page data
  const fetchDonationData = async () => {
    try {
      setLoading(true);
      setError(null);
      
      const [programsData, storiesData, metricsData] = await Promise.all([
        programsApi.getPrograms(),
        impactApi.getSuccessStories(3), // Get first 3 stories
  ...
  const initiateFlutterwavePayment = () => {
    ...
    const selectedCauseData = dynamicCauses.find(cause => cause.id === selectedCause);
  ...
  const getProgramIcon = (category: string) => {
    switch (category) {
      ...
      case 'community_development': return <Users className="w-6 h-6" />;
      case 'healthcare': return <Heart className="w-6 h-6" />;
      case 'environment': return <Lightbulb className="w-6 h-6" />;
      default: return <Star className="w-6 h-6" />;
    }
  };

  // Generate causes from real program data
  const dynamicCauses = programs.length > 0 
    ? [
        ...programs.slice(0, 3).map(program => ({
          id: program.id || program.title.toLowerCase().replace(/\s+/g, '-'),
          title: program.title,
          description: program.description.length > 60 
            ? program.description.substring(0, 60) + '...' 
            : program.description,
          icon: getProgramIcon(program.category),
          color: program.color || 'from-blue-400 to-blue-600',
          impact: `Support ${program.participants || 'youth'} in ${program.title}`
        })),
        {
          id: 'general',
  ...
  // Transform real success stories for display
  const displayImpactStories = successStories.length > 0 
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600 text-lg">Loading donation information...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="max-w-6xl mx-auto text-center">
            <div className={`transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
              ...
              <div className="grid md:grid-cols-3 gap-6 mt-12">
                {(impactMetrics.length >= 3 ? impactMetrics.slice(0, 3).map(metric => ({
              ...
              <div className="space-y-8">
                <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-white/20">
                  ...
                  <div className="space-y-6">
                    {displayImpactStories.map((story, index) => (
          ...
          <div className="max-w-6xl mx-auto">
            <div className="bg-white/60 backdrop-blur-sm rounded-3xl p-12 shadow-2xl border border-white/20">
              ...
              <div className="grid md:grid-cols-4 gap-8">
                {(impactMetrics.length > 0 ? impactMetrics.slice(0, 4).map(metric => ({
                  number: metric.number,
                  label: metric.label,
                  icon: metric.icon
                })) : [
